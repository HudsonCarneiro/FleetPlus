...
exports.exportDeliveriesReport = async (req, res) => {
  try {
      ...
      const reportLines = deliveriesWithDetails.map((supply) => {
        return `
        Data: ${supply.deliveryDate}
        Status: ${supply.status}
        Urgência: ${supply.urgency}
        Motorista: ${supply.Driver?.name || 'Desconhecido'}
        Veículo: ${supply.Vehicle?.model || 'Desconhecido'}
        Placa: (${supply.Vehicle?.plate || ''})
      ...
      res.download(filePath, `relatorio-ordensDeEntrega-${userId}.txt`, () => {
