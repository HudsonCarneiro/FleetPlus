...
exports.getUserAll = async (req, res) => {
  try {
    const users = await User.findAll({
      attributes: ['id' , 'name', 'cpf', 'phone', 'cep', 'email', 'status'], 
    });
...
exports.createUser = async (req, res) => {
  try {
    const { name, cpf, phone, cep, number, road, city, state, email, password, status } = req.body;

    if (!name || !cpf || !phone || !cep || !number || !road || !city || !state ||
